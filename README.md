...
时，钩子毫秒级派发一个后台命名进程（钩子本身约 0.05s 返回，不阻塞发消息）。**数据层**在几秒内就有名字了（实测
提交后 3 秒写入引擎库与任务列表）。**但界面不一定马上变**：桌面端只在收到引擎事件
`session.titleUpdated` 时才更新会话标题（app.asar 的 `applyTitleChange`），外部写库
触发不了这个事件；而引擎自己只在第一轮结束时改名。所以界面上看到的首次更新，通常还是
第一轮结束那一刻——这一点见「已知限制」。
...
- **桌面端列表只认引擎事件**：App 在收到引擎的 `session.titleUpdated` 时才更新标题与
  列表（app.asar 的 `applyTitleChange`），外部写库不会触发它，也没有对外的控制通道
  （引擎是 App 的 stdio 子进程，无 socket/端口）。因此本插件写的标题在数据层立刻生效，
  但界面要等列表重新加载（重启 App、切换工作区）或引擎自己改名时才显示。对「任务跑得久
  想中途从外面看名字」这个诉求，插件目前只能保证数据层已就位，不保证界面即时刷新。
- 内置命名会在第一轮结束时再生成一次标题（它的检查只挡 `custom`），届时会覆盖本插件的
  早期标题——实测就是这样：插件 3 秒写入的名字，93 秒后被引擎版本替换。
